		...
		u32 new_val, int type)
{
	...
	if (fb_dev->panel_if_type != SPRDFB_PANEL_IF_DPI) {
		pr_err("[LCD] %s, panel if should be DPI\n", __func__);
		return -EINVAL;
	}
	if (new_val <= 0 || !new_pclk) {
		pr_err("[LCD] %s, invalid parameter (%d, %d)\n",
	...
	if (fb_dev->panel->type == LCD_MODE_DSI) {
		...
		pr_err("[LCD] %s, unexpected panel type (%d)\n",
	...
	case SPRDFB_DYNAMIC_FPS:
		if (new_val < LCD_MIN_FPS || new_val > LCD_MAX_FPS) {
			pr_err("[LCD] %s, out of range (%d), range(%d, %d)\n",
					__func__, new_val,
	...
	default:
		pr_err("[LCD] %s, unsupported type (%d)\n", __func__, type);
		*new_pclk = 0;
		return -EINVAL;
	}
	return 0;
...
static irqreturn_t dispc_isr(int irq, void *data)
{
	...
	pr_debug("[LCD] %s, status : 0x%X\n", __func__, reg_val);

	if (reg_val & DISPC_INT_ERR_MASK) {
		pr_err("[LCD] %s, dispc underflow : 0x%x\n", __func__, reg_val);
	...
	if (done) {
		...
		pr_debug("[LCD] %s, done interrupt (0x%x)\n", __func__, reg_val);
	}
...
static void dump_dispc_reg(void)
{
	...
	for (i = 0; i < 256; i += 16) {
		pr_info("[LCD] sprdfb: %x: 0x%x, 0x%x, 0x%x, 0x%x\n",
...
static int32_t dispc_clk_init(struct sprdfb_device *dev)
{
	...
	ret = of_property_read_u32_array(dev->of_dev->of_node,
			"clock-src", clk_src, 3);
	if (ret) {
		pr_err("[LCD] %s, read clock-src failed (%d)\n", __func__, ret);
		return -1;
	}
	...
	ret = of_property_read_u32(dev->of_dev->of_node, "dpi_clk_div",
			&def_dpi_clk_div);
	if (ret) {
		pr_err("[LCD] %s, read dpi_clk_div failed (%d)\n", __func__, ret);
		return -1;
	}
	...
	if (IS_ERR(clk_parent1)) {
		pr_err("[LCD] %s, failed to get parent1 clock (%s)\n",
				__func__, DISPC_CLOCK_PARENT);
		return -1;
	}
	pr_debug("[LCD] %s, get parent1 clock (%s)\n",
			__func__, DISPC_CLOCK_PARENT);
	...
	if (IS_ERR(clk_parent2)) {
		pr_err("[LCD] %s, failed to get parent2 clock (%s)\n",
				__func__, DISPC_DBI_CLOCK_PARENT);
		return -1;
	}
	pr_debug("[LCD] %s, get parent2 clock (%s)\n",
			__func__, DISPC_DBI_CLOCK_PARENT);
	...
	if (IS_ERR(clk_parent3)) {
		pr_err("[LCD] %s, failed to get parent3 clock (%s)\n",
				__func__, DISPC_DPI_CLOCK_PARENT);
		return -1;
	}
	pr_debug("[LCD] %s, get parent3 clock (%s)\n",
			__func__, DISPC_DPI_CLOCK_PARENT);
	...
	if (IS_ERR(clk_parent4)) {
		pr_err("[LCD] %s, failed to get parent4 clock (%s)\n",
				__func__, DISPC_EMC_EN_PARENT);
		return -1;
	}
	pr_debug("[LCD] %s, get parent4 clock (%s)\n",
			__func__, DISPC_EMC_EN_PARENT);
	...
	if (IS_ERR(dispc_ctx.clk_dispc)) {
		pr_err("[LCD] %s, failed to get pll clock (%s)\n",
				__func__, DISPC_PLL_CLK);
		return -1;
	}
	pr_debug("[LCD] %s, get pll clock (%s)\n", __func__, DISPC_PLL_CLK);
	...
	if (IS_ERR(dispc_ctx.clk_dispc_dbi)) {
		pr_err("[LCD] %s, failed to get dbi clock (%s)\n",
				__func__, DISPC_DBI_CLK);
		return -1;
	}
	pr_debug("[LCD] %s, get dbi clock (%s)\n", __func__, DISPC_DBI_CLK);
	...
	if (IS_ERR(dispc_ctx.clk_dispc_dpi)) {
		pr_err("[LCD] %s, failed to get dpi clock (%s)\n",
				__func__, DISPC_DPI_CLK);
		return -1;
	}
	pr_debug("[LCD] %s, get dpi clock (%s)\n", __func__, DISPC_DPI_CLK);
	...
	if (IS_ERR(dispc_ctx.clk_dispc_emc)) {
		pr_err("[LCD] %s, failed to get emc clock (%s)\n",
				__func__, DISPC_EMC_CLK);
		return -1;
	}
	pr_debug("[LCD] %s, get emc clock (%s)\n", __func__, DISPC_EMC_CLK);

	ret = clk_set_parent(dispc_ctx.clk_dispc, clk_parent1);
	if (ret)
		pr_err("[LCD] %s, failed to set dispc clk parent\n", __func__);
#ifdef CONFIG_OF
...
#endif
	if (ret)
		pr_err("[LCD] %s, failed to set dispc clk rate\n", __func__);

	ret = clk_set_parent(dispc_ctx.clk_dispc_dbi, clk_parent2);
	if (ret)
		pr_err("[LCD] %s, failed to set dbi clk parent\n", __func__);
#ifdef CONFIG_OF
...
#endif
	if (ret)
		pr_err("[LCD] %s, failed to set dbi clk rate\n", __func__);

	ret = clk_set_parent(dispc_ctx.clk_dispc_dpi, clk_parent3);
	if (ret)
		pr_err("[LCD] %s, failed to set dpi clk parent\n", __func__);

	ret = clk_set_parent(dispc_ctx.clk_dispc_emc, clk_parent4);
	if (ret)
		pr_err("[LCD] %s, failed to set emc clk parent\n", __func__);
...
#endif
	if (ret) {
		pr_err("[LCD] %s, failed to enable dispc emc clk\n", __func__);
		ret = -1;
	}

	ret = sprdfb_dispc_clk_enable(&dispc_ctx, SPRDFB_DYNAMIC_CLK_FORCE);
	if (ret) {
		pr_err("[LCD] %s, failed to enable dispc_clk\n", __func__);
		return -1;
	}
	pr_debug("[LCD] %s, enable dispc_clk\n", __func__);
		...
		unsigned long event, void *dummy)
{
	if (event != SC_CPUIDLE_PREPARE) {
		pr_err("[LCD] %s, invalid cpuidle event (%lu)\n",
				__func__, event);
		return 0;
	}
...
static int32_t sprdfb_dispc_module_init(struct sprdfb_device *dev)
{
	...
	if (ret)
		pr_err("[LCD] %s, light sleep register notifier failed\n", __func__);
#endif
	return 0;
}
...
static int32_t sprdfb_dispc_early_init(struct sprdfb_device *dev)
{
	...
	if (ret) {
		pr_warn("[LCD] %s, dispc_clk_init failed (%d)\n", __func__, ret);
		return -1;
	}
	...
	if (!dispc_ctx.is_inited) {
		/* init */
		if (dev->panel_ready) {
			pr_info("[LCD] %s, dispc already initialized\n", __func__);
			dispc_ctx.is_first_frame = false;
		} else {
			pr_debug("[LCD] %s, dispc not initialized\n", __func__);
			dispc_reset();
			dispc_module_enable();
			dispc_ctx.is_first_frame = true;
		}
		...
		SPRDFB_DYNAMIC_CLK_SWITCH_E clock_switch_type)
{
	...
	if (dispc_ctx_ptr->clk_is_open && is_need_disable) {
		pr_info("[LCD] %s, disable dispc clk\n", __func__);
#ifdef CONFIG_OF
	...
	pr_debug("[LCD] %s, type=%d refresh=%d, count=%d\n",
		...
		SPRDFB_DYNAMIC_CLK_SWITCH_E clock_switch_type)
{
	...
	if (!dispc_ctx_ptr->clk_is_open) {
		pr_info("[LCD] %s, enable dispc clk\n", __func__);
#ifdef CONFIG_OF
...
#endif
		if (ret) {
			pr_err("[LCD] %s, failed to enable dispc_clk (%d)\n",
					__func__, ret);
			ret = -1;
			goto ERROR_CLK_ENABLE;
		}
		is_dispc_enable = true;
		pr_info("[LCD] %s, dispc clk enabled\n", __func__);
#ifdef CONFIG_OF
...
#endif
		if (ret) {
			pr_err("[LCD] %s, failed to enable dpi clk(%d)\n",
					__func__, ret);
			ret = -1;
			goto ERROR_CLK_ENABLE;
		}
		is_dispc_dpi_enable = true;
		pr_info("[LCD] %s, dpi clk enabled\n", __func__);
#ifdef CONFIG_OF
...
#endif
		if (ret) {
			pr_err("[LCD] %s, failed to enable dbi clk(%d)\n",
					__func__, ret);
			ret = -1;
			goto ERROR_CLK_ENABLE;
		}
		dispc_ctx_ptr->clk_is_open = true;
		pr_info("[LCD] %s, dbi clk enabled\n", __func__);
	}
	...
	pr_debug("[LCD] %s, type=%d refresh=%d, count=%d, ret=%d\n",
			__func__, clock_switch_type,
			dispc_ctx_ptr->clk_is_refreshing,
...
static int32_t sprdfb_dispc_refresh(struct sprdfb_device *dev)
{
	...
	pr_debug("[LCD] %s, got sync\n", __func__);
#ifdef CONFIG_FB_MMAP_CACHED
	if (dispc_ctx.vma) {
		pr_debug("[LCD] %s, dispc_ctx.vma=0x%x\n", dispc_ctx.vma);
	...
	if (!dev->ESD_work_start) {
		pr_info("[LCD] %s, schedule esd work\n", __func__);
...
ERROR_REFRESH:
	...
	if (dev->logo_buffer_addr_v) {
		pr_info("[LCD] %s, free logo proc buffer\n", __func__);
...
static int32_t sprdfb_dispc_suspend(struct sprdfb_device *dev)
{
...
#ifdef CONFIG_MACH_VIVALTO3MVEML3G
	...
	up(&dev->refresh_lock);
#endif
#ifndef CONFIG_MACH_VIVALTO3MVEML3G
	dispc_write(0, DISPC_INT_EN);
	msleep(20); /*fps 1frame >  previous interrupt done time*/
#endif
	dispc_stop(dev);
#ifdef CONFIG_MACH_VIVALTO3MVEML3G
	dispc_write(0, DISPC_INT_EN);
...
static int32_t sprdfb_dispc_check_esd(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DBI == dev->panel_if_type) {
		pr_err("[LCD] %s, unsupported in dbi\n", __func__);
		ret = -1;
...
static int32_t sprdfb_dispc_esd_reset(struct sprdfb_device *dev)
{
	...
	if (!dev->enable) {
		pr_warn("[LCD] %s, dispc not enabled\n", __func__);
		mutex_unlock(&dispc_lock);
		return;
	}
...
static int overlay_start(struct sprdfb_device *dev, uint32_t layer_index)
{
	...
	if (SPRD_OVERLAY_STATUS_ON != dispc_ctx.overlay_state) {
		pr_err("[LCD] sprdfb: %s overlay is not opened\n", __func__);
		return -1;
	}
		...
		int y_endian, int uv_endian, bool rb_switch)
{
	...
	pr_debug("[LCD] %s, %d, (%d, %d,%d,%d), 0x%x\n", __func__,
			type, rect->x, rect->y, rect->h, rect->w,
			(unsigned int)buffer);

	if (SPRD_OVERLAY_STATUS_ON != dispc_ctx.overlay_state) {
		pr_err("[LCD] %s, error : overlay off\n", __func__);
		return -1;
	}

	if (type >= SPRD_DATA_TYPE_LIMIT) {
		pr_err("[LCD] %s error : invalid type(%d)\n",
				__func__, type);
		return -1;
	}
	...
	if ((y_endian >= SPRD_IMG_DATA_ENDIAN_LIMIT) ||
		...
		pr_err("[LCD] %s, error : invalid y uv endian\n", __func__);
		return -1;
	}
		...
		int y_endian, int uv_endian, bool rb_switch)
{
	...
	if (SPRD_OVERLAY_STATUS_ON != dispc_ctx.overlay_state) {
		pr_err("[LCD] %s, overlay not opened!! (%d)\n",
				__func__, dispc_ctx.overlay_state);
		return -1;
	}

	if ((type >= SPRD_DATA_TYPE_LIMIT) || (type <= SPRD_DATA_TYPE_YUV400)) {
		pr_err("[LCD] %s, invalid type (%d)\n", __func__, type);
		return -1;
	}

	if (y_endian >= SPRD_IMG_DATA_ENDIAN_LIMIT) {
		pr_err("[LCD] %s, invalid y_endian (%d)\n", __func__, y_endian);
		return -1;
	}
...
static int overlay_close(struct sprdfb_device *dev)
{
	if (SPRD_OVERLAY_STATUS_OFF == dispc_ctx.overlay_state) {
		pr_warn("[LCD] %s, overlay already closed\n", __func__);
		return 0;
	}
		...
		struct overlay_info *info, int enable)
{
	...
	if (enable) {
		if (!info) {
			pr_err("[LCD] %s, invalid parameter\n", __func__);
			goto ERROR_ENABLE_OVERLAY;
		}
		down(&dev->refresh_lock);
		is_refresh_lock_down = true;
		if (!dev->enable) {
			pr_err("[LCD] %s, dispc inactive state\n", __func__);
			goto ERROR_ENABLE_OVERLAY;
		}

		if (dispc_sync(dev)) {
			pr_err("[LCD] %s, dispc_sync timeout\n", __func__);
			goto ERROR_ENABLE_OVERLAY;
		}
#ifdef CONFIG_FB_DYNAMIC_CLK_SUPPORT
		if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type) {
			if (sprdfb_dispc_clk_enable(&dispc_ctx,
						SPRDFB_DYNAMIC_CLK_COUNT)) {
				pr_warn("[LCD] %s, clk enable failed\n", __func__);
				goto ERROR_ENABLE_OVERLAY;
			}
			is_clk_enable = true;
		}
		...
		struct overlay_display *setting)
{
	...
	pr_debug("[LCD] %s, layer:%d, (%d, %d,%d,%d)\n", __func__,
	...
	if (SPRD_OVERLAY_STATUS_ON == dispc_ctx.overlay_state) {
		if (overlay_start(dev, setting->layer_index)) {
			pr_err("[LCD] %s overlay start failed\n", __func__);
			goto ERROR_DISPLAY_OVERLAY;
		}
	}
	...
	if (SPRDFB_PANEL_IF_DPI == dev->panel_if_type) {
		if (!dispc_ctx.is_first_frame) {
			...
			if (!ret) { /* time out */
				...
				pr_err("[LCD] %s, vsync timeout (0x%x, 0x%x)\n",
		...
		unsigned int state)
{
	...
	pr_info("[LCD] %s, DMC change freq(%u)\n", state);
	if (SPRDFB_PANEL_IF_DPI == dev->panel_if_type) {
		down(&dev->refresh_lock);
...
static int32_t sprdfb_dispc_refresh_logo(struct sprdfb_device *dev)
{
	...
	if (SPRDFB_PANEL_IF_DPI != dev->panel_if_type) {
		pr_err("[LCD] %s, panel interface is not dpi\n", __func__);
		return 0;
	}
	...
	while (!(dispc_read(DISPC_INT_RAW) & 0x10) && --cnt)
			udelay(1000);
	if (!cnt)
		pr_err("[LCD] %s, wait update done (status :%d) - timeout!!\n",
...
int32_t sprdfb_dispc_logo_proc(struct sprdfb_device *dev)
{
	...
	if (!dev) {
		pr_err("[LCD] %s, no device\n", __func__);
		return -1;
	}

	if (!lcd_base_from_uboot) {
		pr_warn("[LCD] %s, no lcd_base\n", __func__);
		return -1;
	}
	...
	if (dev->panel->display_width && dev->panel->display_height) {
		logo_size = dev->panel->display_width *
			dev->panel->display_height * (dev->bpp / 8);
	} else
#endif
	...
	dev->logo_buffer_addr_v =
		__get_free_pages(GFP_ATOMIC | __GFP_ZERO,
			get_order(logo_size));
	if (!dev->logo_buffer_addr_v) {
		pr_err("[LCD] %s, allocation failed\n", __func__);
		return -1;
	}
	...
	if (!logo_src_v || !logo_dst_v) {
		pr_err("[LCD] %s, unable to map logo virt(s:0x%08x, d:0x%08x)\n",
	...
	pr_info("[LCD] %s, copy logo - src p:0x%08X dst p:0x%08X size %dk\n",
